...
async function run(output=response(),status=200,approved=true){
 ...
 return buildNewsSnapshot({now,sourceResults:[{id:'cnfin',name:'新华财经',ok:true,items:[raw]}],enrichmentOptions:{apiKey:'test-secret',fetchImpl}});
}
