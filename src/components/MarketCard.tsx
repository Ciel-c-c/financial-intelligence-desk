  ...
  return (
    <article className="market-card">
      ...
      <div className="market-card-body">
        <div className="market-quote"><h3>{item.indexName}</h3><strong>{item.value}</strong></div>
        <p className={`market-change ${rising ? 'positive' : 'negative'}`}><span aria-hidden="true">{rising ? '↑' : '↓'}</span> {rising ? '+' : ''}{item.changePercent.toFixed(2)}% · {rising ? '上涨' : '下跌'}</p>
      </div>
