...
export function NewsFeedSections({snapshot,loading,error}:{snapshot:NewsFeedSnapshot;loading:boolean;error?:string}) {
  ...
  const filtered = filterLiveNews(all,filters);
  ...
  return <section className="live-news-feed"><NewsFeedStatus snapshot={snapshot}/>{error && <p className="empty-state">{error}</p>}<NewsFilters value={filters} onChange={setFilters}/>
