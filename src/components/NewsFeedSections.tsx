...
export function NewsFeedSections({snapshot,loading,error,showFilters=true}:{snapshot:NewsFeedSnapshot;loading:boolean;error?:string;showFilters?:boolean}) {
  ...
  return <section className="live-news-feed"><NewsFeedStatus snapshot={snapshot}/>{error && <p className="empty-state">{error}</p>}{showFilters && <NewsFilters value={filters} onChange={setFilters}/>}
