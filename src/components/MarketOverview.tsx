...
export function MarketOverview({ snapshot, activeGroup, onGroupChange }: { snapshot?: MarketOverviewSnapshot; activeGroup: MarketGroup; onGroupChange: (group: MarketGroup) => void }) {
  ...
  return <>
    <section className="market-hero"><div className="market-hero-head"><div><p className="eyebrow">MARKET OVERVIEW / 市场总览</p><h1>{groupLabels[activeGroup]}概览</h1></div><span className="market-status">{health ? stateLabels[items[0]?.marketState ?? 'delayed'] : '等待数据'}</span></div><div className="market-summary"><strong>{items.length ? `${items.length} 个观察项，${up} 个上涨、${down} 个下跌` : '暂无可靠市场数据'}</strong><p>{items.length ? '以下只描述已验证数据；没有可靠证据时不推断涨跌原因。' : health?.fallbackReason ?? '数据源尚未通过验证或本轮获取失败。'}</p></div><small>数据截至 {formatTime(snapshot?.dataAsOf)} · 状态：{snapshot?.status ?? 'unavailable'}</small></section>
