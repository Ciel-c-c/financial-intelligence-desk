...
export function NewsArticle({item,data}:{item:NewsItem;data?:SoWhatData}) {
  ...
  return (
    <main className="page detail-page">
      ...
      <section className="reading-card"><p className="eyebrow">{item.mode === '演示' ? '演示阅读段落' : '完整正文已读取 · 中文解读'}</p><p>{item.excerpt}</p><div className="terms">{terms.map((term) => <TermExplanation key={term.id} item={term} />)}</div></section>
      ...
      <SoWhatSection data={data ?? soWhatForNews(item)} />
