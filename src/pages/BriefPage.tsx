...
export function BriefPage() {
  ...
  return <main className="page inner-page">
    ...
    {!brief || brief.status === 'unavailable' ? <p className="empty-state">暂无足够的新事实生成可靠简报。<button onClick={refresh}>检查更新</button></p> : <>
      <section><h2 className="display-heading">近 24 小时重点事件</h2><div className="brief-list">{recentStories.length ? recentStories.map((item,index) => <Link to={`/news/${item.id}`} key={item.id}><span>{String(index+1).padStart(2,'0')}</span><strong>{item.title}</strong><small>发布：{time(item.publishedAt)} · {item.sourceName}<br/>查看新闻与解读 →</small></Link>) : <p className="empty-state">暂无近 24 小时新闻，不将旧简报内容作为今日重点。</p>}</div></section>
      {backgroundStories.length > 0 && <details className="watch-card" open={!recentStories.length}><summary><h2>此前报道 / 背景参考</h2></summary><p>保留此前简报，未核实有新进展。</p><div className="brief-list">{backgroundStories.map((item,index) => <Link to={`/news/${item.id}`} key={item.id}><span>{String(index+1).padStart(2,'0')}</span><strong>{item.title}</strong><small>新闻发布：{time(item.publishedAt)} · {item.sourceName}</small></Link>)}</div></details>}
