...
export function BriefPage(){
  ...
  return <main className="page inner-page">
    <header className="page-title"><p className="eyebrow">DAILY MARKET BRIEF</p><h1>每日市场简报</h1><time>{date??'等待更新'}</time><p>{recent.length?stories[0].editorial!.item.summary:'暂无近24小时已核验完整新闻；以下保留此前报道，不作为今日重点。'}</p><small>生成于 {brief?new Date(brief.generatedAt).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai'}):'等待简报生成'}（北京时间） · 新闻日期逐条标注</small></header>
    <section><h2 className="display-heading">{recent.length?'重点事件':'此前重点事件 · 背景参考'}</h2><div className="brief-list">{stories.length?stories.map((item,index)=><BriefStoryCard key={item.id} index={index} item={item} story={{id:item.id,title:item.editorial!.item.title,publishedAt:item.publishedAt,sourceName:item.sourceName,sourceUrl:item.canonicalUrl}}/>):<p className="empty-state">{loading?'正在读取完整新闻…':'暂无足够的新事实生成可靠简报；未读取完整正文的新闻不展示。'}</p>}</div></section>
