...
export async function buildNewsSnapshot({now,sourceResults,previous,enrichmentOptions={},reviewedNews=[]}){
  ...
  if(enrichmentOptions.apiKey) sourceHealth.push({id:'cerebras-editorial',name:'Cerebras 完整正文解读',status:state.stopped||state.failures?'error':'ok',itemCount:state.generated,...(state.reason?{error:state.reason}:{})});
